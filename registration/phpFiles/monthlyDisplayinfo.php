<?php 
/*********************************************************************
 *This is a test file to demonstrate monthly reports based on PLC's  *
...
<body>";
//get the date and the begining of the month for sql query
...
function monthlyMaxQuery($array){
...
//string
function monthlyMinQuery($array){
...
function bestOee($name,$link,$max){
	...
	if($max){
		$query=monthlyMaxQuery($temp);
	}
	else{
		$query=monthlyMinQuery($temp);	
	}
	//finish the query string from the start of the month to now
	$query.="from ".$name." where timestamp between '".$start_month."' and '".$today."';";
	$results=mysqli_query($link,$query);
	$row=mysqli_fetch_row($results);
	//create the table with the results from the query
	echo "<table class='monthly'>";
	echo "</tr><tr>";
	$i=0;
	$add=0;
	foreach($row as $item){
	...
	if($add>0){
		...
		if($max){
			echo "Monthly Max OEE of <u>";	
		}
		else{
			echo "Monthly Min OEE of <u>";
		}
...
function displayMonthlyInfo($name,$link){
	global $start_month,$today;
	$query="show columns from ".$name.";";
	$results=mysqli_query($link,$query);
	$temp=[];
	...
	if($add>0){
		echo "</tr><tr> <td colspan=".($add).">Monthly OEE of <u>".str_replace("_","",$name)."</u></td>";
	}
	echo "</table>";
...
echo "<div class ='head'>
	...
	<td align='center'><h1>T.A. Monthly Report</h1><td>
...
foreach($tables as $col){
	displayMonthlyInfo($col,$link);
	displayMonthlyFaults($col,$link);
	bestOee($col,$link,1);
	bestOee($col,$link,0);
}
//end the webpage
