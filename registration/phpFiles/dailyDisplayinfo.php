<?php 
/*********************************************************************
 *This is a test file to demonstrate daily reports based on PLC's  *
...
<body>";
//get the date and the begining of the day for sql query
...
function bestOee($name,$link,$max){
	...
	if($max){
		$query=dailyMaxQuery($temp);
	}
	else{
		$query=dailyMinQuery($temp);	
	}
	//finish the query string from the start of the day to now
	$query.="from ".$name." where timestamp between '".$start_day."' and '".$today."';";
	$results=mysqli_query($link,$query);
	$row=mysqli_fetch_row($results);
	//create the table with the results from the query
	echo "<table class='daily'>";
	echo "</tr><tr>";
	$i=0;
	$add=0;
	foreach($row as $item){
	...
	if($add>0){
		...
		if($max){
			echo "Daily Max OEE of <u>";	
		}
		else{
			echo "Daily Min OEE of <u>";
		}
...
function displayDailyInfo($name,$link){
	global $start_day,$today;
	$query="show columns from ".$name.";";
	$results=mysqli_query($link,$query);
	$temp=[];
	...
	if($add>0){
		echo "</tr><tr> <td colspan=".($add).">Daily OEE of <u>".str_replace("_","",$name)."</u></td>";
	}
	echo "</table>";
...
echo "<div class ='head'>
	...
	<td align='center'><h1>T.A. Daily Report</h1><td>
...
foreach($tables as $col){
	displayDailyInfo($col,$link);
	displayDailyFaults($col,$link);
	bestOee($col,$link,1);
	bestOee($col,$link,0);
}
//end the webpage
